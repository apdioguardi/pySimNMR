...
def _joblib_kwargs_from_env():
    """Select a sandbox-friendly parallel strategy based on env vars.

    If PY_SIMNMR_THREADS is set (e.g., by tests), prefer threads to avoid
    process-based backends that can be blocked in CI/sandboxes.
    """
    prefer = "processes"
    n_jobs = -1
    if str(os.environ.get("PY_SIMNMR_THREADS", "")).strip().lower() in {"1", "true", "yes", "on"}:
        prefer = "threads"
        # Cap the default thread count to keep tests snappy
        try:
            n_jobs = min(4, int(os.environ.get("PY_SIMNMR_N_JOBS", 0)) or _CPU_COUNT)
        except Exception:
            n_jobs = min(4, _CPU_COUNT)
    if str(os.environ.get("PY_SIMNMR_DISABLE_PARALLEL", "")).strip().lower() in {"1", "true", "yes", "on"}:
        prefer = "threads"
        n_jobs = 1
    return {"n_jobs": n_jobs, "prefer": prefer}

def _rotation_matrix_cache_path():
    """Return the absolute path where rotation_matrices.h5 should live."""
    env_path = os.environ.get("PYSIMNMR_ROTATION_CACHE")
    if env_path:
        return os.path.abspath(env_path)
    output_dir = os.environ.get("PYSIMNMR_OUTPUT_DIR", "output")
    return os.path.abspath(os.path.join(output_dir, "rotation_matrices.h5"))
...
class SimNMR(object):
    ...
    def elevels_vs_field_ed(self, 
        ...
        n_orient = r.shape[0]
        # Legacy callers (old CLIs, notebooks) pass Hint in a variety of shapes
        # (single 3-vector, 3xn arrays, cached stacks).  Normalise everything to a
        # consistent (n_orient, 3, 1) array so the Zeeman term can broadcast cleanly
        # over the rotation matrices without surprises.
        Hint = None
        if Hint is None:
            Hint = np.zeros((n_orient, 3, 1), dtype=float)
        else:
            Hint = np.asarray(Hint, dtype=float)
            if Hint.ndim == 1:
                if Hint.size != 3:
                    raise ValueError('Hint vector must have three components')
                Hint = Hint[np.newaxis, :, np.newaxis]
            elif Hint.ndim == 2:
                if Hint.shape[1] == 3:
                    Hint = Hint[:, :, np.newaxis]
                elif Hint.shape[0] == 3:
                    Hint = Hint.T[:, :, np.newaxis]
                else:
                    Hint = Hint.reshape(-1, 3, 1)
            elif Hint.ndim == 3:
                if Hint.shape[-1] != 1:
                    Hint = Hint.reshape(Hint.shape[0], Hint.shape[1], 1)
            else:
                Hint = Hint.reshape(-1, 3, 1)
            if Hint.shape[0] == 1 and n_orient > 1:
                Hint = np.repeat(Hint, n_orient, axis=0)
            if Hint.shape[0] != n_orient:
                raise ValueError('Hint array must match rotation count')
    ...
    def exact_diag(self, 
        ...
        if isinstance(H0, np.ndarray):
            H0_array = H0.astype(float, copy=False)
        elif isinstance(H0, list):
            H0_array = np.array(H0, dtype=float)
        elif isinstance(H0, lmfit_Parameter):
            H0_array = np.array([H0.value], dtype=float)
        else:
            H0_array = np.array([float(H0)], dtype=float)
        # Store H0 as a column vector per orientation so the Knight-shifted
        # Zeeman term can be applied through a single matrix multiply and later
        # reshaped back to the original field grid.
        H0vec = np.zeros(shape=(H0_array.shape[0], 3, 1)) 
        H0vec[:,2,0]=H0_array # populate the z directions of the H0vec from the input array
        eyePlusK = (II3 + Ktensor)
        KdotH0 = eyePlusK@H0vec
        ...
        ## allow for an internal magnetic field due to hyperfine coupling to magnetic order
        # may want to modify this to actually be S@A@Ivec in the future 
        # Rotate the internal field into the EFG PAS so Hint components line up
        # with the Ix/Iy/Iz basis used for the Hamiltonian.
        Hint = r@Hint
        if Hint.ndim == 3 and Hint.shape[-1] == 1:
            Hint = Hint[:, :, 0]
    ...
    def random_rotation_matrices(self,
        ...
        saves:    
        rotation_matrices.h5 file is written to the output directory (or the path
            pointed to by ``PYSIMNMR_ROTATION_CACHE``) if recalc_random_samples = True OR 
            if recalc_random_samples = False, but the file rotation_matrices.h5 cannot be found there.
            Or if rotation_matrices.h5 exists, has the correct number of samples, but
        ...
        t0 = time.time()
        ################################################################################
        rotation_matrix_path = _rotation_matrix_cache_path()
        rotation_matrix_dir = os.path.dirname(rotation_matrix_path)
        legacy_path = os.path.abspath('rotation_matrices.h5')
        if (rotation_matrix_path != legacy_path
                and os.path.isfile(legacy_path)
                and not os.path.isfile(rotation_matrix_path)):
            if rotation_matrix_dir:
                os.makedirs(rotation_matrix_dir, exist_ok=True)
            print(f'Moving existing rotation_matrices.h5 to {rotation_matrix_path} to keep caches in the output directory.')
            shutil.move(legacy_path, rotation_matrix_path)

        rotation_matrix_file_exists = os.path.isfile(rotation_matrix_path)
        if not recalc_random_samples and not rotation_matrix_file_exists:
            if rotation_matrix_dir:
                os.makedirs(rotation_matrix_dir, exist_ok=True)
            print('Could not find rotation_matrices.h5 file in the output directory. Generating and saving new random rotation matrices...')
            recalc_random_samples = True

        if rotation_matrix_file_exists:
            with h5py.File(rotation_matrix_path, 'r') as h5f:
    ...
    New rotation matrices will be generated.""")        
        ...
        if recalc_random_samples:
            ...
            if rotation_matrix_dir:
                os.makedirs(rotation_matrix_dir, exist_ok=True)
            with h5py.File(rotation_matrix_path, 'w') as h5f:
        ...
        for isotope in isotope_list:
            ...
            if recalc_random_samples:
                ...
                r_spin, ri_spin = sim.generate_r_spin_matrices(phi_z_array,
                                                            theta_xp_array,
                                                            psi_zp_array)
                with h5py.File(rotation_matrix_path, 'a') as h5f:
    ...
    def field_spec_edpp(self,
        ...
        freq_array, H0_array_out, prob_array, trans_array = self.freq_prob_trans_edpp_HS_parallel(
            H0_array,
            Ka,
            Kb,
            Kc,
...
#)
    ...
    def freq_prob_trans_edpp_HS_parallel(self, 
        ...
        """
        # Parallel execution using joblib (prefer threads under tests/sandbox)
        parallel_kwargs = _joblib_kwargs_from_env()
        if progress_bar is not None:
            parallel_kwargs.setdefault("batch_size", 1)
        n_jobs = parallel_kwargs.get("n_jobs", -1)
        def _run_sequential() -> list:
            seq_results = []
            for H0 in H0_array:
                seq_results.append(
                    self.freq_prob_trans_edpp_HS(
        ...
        if progress_bar is not None and n_jobs == 1:
            results = _run_sequential()
        else:
            executor = Parallel(**parallel_kwargs)
            tasks = (
            ...
            try:
                if progress_bar is not None:
                    with joblib_progress(progress_bar):
    ...
    def sec_ord_field_pp(self,
        ...
        if I0==0.5: #ahoy! this should be boolean...
            ...
            if min_field==None or max_field==None:
                ...
                min_range=min_field
                max_range=max_field
            bin_edges = np.linspace(min_range, max_range, int(nbins) + 1)
            # Accumulate histograms chunk-by-chunk so progress updates can fire regularly.
            hist = np.zeros(int(nbins))
            chunk = max(1, nrands // 20)
            for start in range(0, field_array.size, chunk):
                end = min(start + chunk, field_array.size)
                chunk_fields = field_array[start:end]
                hist_chunk, _ = np.histogram(chunk_fields, bins=bin_edges)
                hist += hist_chunk
                _update_progress(len(chunk_fields))
        else:
            #print("Running second order calc...")
            dim = int((I0+1/2)*2)
            m_values = np.array([I0-i for i in range(dim-1)])
            ...
            for start in range(0, field_array.size, chunk):
                ...
                fields_chunk = field_array[start:end]
                probs_chunk = prob_array[start:end]
                hist_chunk, _ = np.histogram(fields_chunk, bins=bin_edges, weights=probs_chunk)
                hist += hist_chunk
                _update_progress(len(fields_chunk))
        bin_edges=bin_edges[:-1]
        histogram = np.column_stack((bin_edges,hist))
        convolved_hist_intensity = self.convolveGaussLor(x_array=bin_edges,
                                                    y_array=hist,
